document.addEventListener("DOMContentLoaded", () => {
  ...
  function pickColor(x, y) {
    ...
    currentColor =
      `#${imgData[0].toString(16).padStart(2, "0")}${imgData[1].toString(16).padStart(2, "0")}${imgData[2].toString(16).padStart(2, "0")}`.toUpperCase();
  ...
  canvas.addEventListener("mousemove", (e) => {
    ...
    if (currentMode === "eraser") {
      ...
      if (currentShape === "line") {
        ...
        ctx.moveTo(startX + (e.offsetX - startX) / 2, startY);
        ctx.lineTo(e.offsetX, e.offsetY);
  ...
  function renderAnalysisResult(analysis) {
    document.getElementById("res-style").innerText =
      analysis.style_name || "未提供";
    document.getElementById("res-era").innerText = analysis.era || "未提供";
    document.getElementById("res-master").innerText = formatMasters(
      analysis.masters,
    );
    document.getElementById("res-review").innerText =
      analysis.curator_review || "未提供";
    document.getElementById("res-history").innerText =
      analysis.art_knowledge || "未提供";
    document.getElementById("res-quote").innerText =
      analysis.artist_quote || "未提供";
  }

  async function submitArtworkForAnalysis() {
    const inputTitle =
      document.getElementById("art-title").value.trim() || "無題";
    const inputMood = document.getElementById("art-inspiration").value.trim();

    document.getElementById("display-art-title").innerText =
      `《${inputTitle}》`;
  ...
  btnRepaint.addEventListener("click", () => {
    ...
    document.getElementById("art-inspiration").value = "";
    initCanvasState();
  ...
  btnGenerateCard.addEventListener("click", () => {
    ...
    document.getElementById("postcard-display-title").innerText = inputTitle
      ? inputTitle
      : "無題";
    postcardCtx.putImageData(
      ctx.getImageData(0, 0, canvas.width, canvas.height),
      0,
      0,
    );
  ...
  btnDownloadJpg.addEventListener("click", () => {
    html2canvas(document.getElementById("postcard-print-area"), {
      scale: 2,
    }).then((canvasImage) => {
  ...
  btnPencil.addEventListener("click", () => {
    ...
    document.querySelector('[data-shape="free"]').classList.add("active");
    toggleEraserPanel(false);
  });
  ...
  document.querySelectorAll(".brush-type-option").forEach((option) => {
    option.addEventListener("click", () => {
      document
        .querySelectorAll(".brush-type-option")
        .forEach((opt) => opt.classList.remove("active"));
      option.classList.add("active");
      currentBrushType = option.getAttribute("data-type");
      currentBrushText.innerHTML =
        option.innerHTML +
  ...
  shapeItems.forEach((item) => {
    item.addEventListener("click", () => {
      ...
      currentShape = item.getAttribute("data-shape");
      currentMode = "pencil";
      clearToolActive();
      btnPencil.classList.add("active");
      toggleEraserPanel(false);
  ...
  colorDots.forEach((dot) => {
    dot.addEventListener("click", () => {
      ...
      if (["eraser", "picker", "bucket"].includes(currentMode)) {
        currentMode = "pencil";
        clearToolActive();
        btnPencil.classList.add("active");
        toggleEraserPanel(false);
